...
from model import CADTransformer, CADTransformer_VAE
...
class TrainerAE(BaseTrainer):
    def build_net(self, cfg):
        if cfg.use_VAE:
            self.net = CADTransformer_VAE(cfg).cuda()
        else:
    ...
    def forward(self, data, epoch):
        ...
        loss_dict = self.loss_func(outputs, epoch)
    ...
    def encode_mem(self, data, is_batch=False):
        """encode into memory state"""
        commands = data['command'].cuda()
        args = data['args'].cuda()
        '''
        # data 확인 코드
        pd = torch.cat((commands.unsqueeze(2), args), 2)
        ...
        if not is_batch:
            commands = commands.unsqueeze(0)
            args = args.unsqueeze(0)
        mem = self.net(commands, args, encode_mem_mode=True)
        return mem
    ...
    def decode_mem(self, z, mems):
        """decode given memory state"""
        outputs = self.net(None, None, z=z, mems=mems, return_tgt=False)
        return outputs
    ...
    def logits2vec(self, outputs, refill_pad=True, to_numpy=True):
        ...
        # EOS_IDX을 찾아서 EOS_IDX - 1의 out_command는 Extrude가 나와야 하므로 설정
        for i in range(out_command.size()[0]):
            try:
                eos_idx_cmd = out_command[i].tolist().index(EOS_IDX)
            except:
                eos_idx_cmd = len(out_command[i]) - 1
                out_command[i][eos_idx_cmd] = 3
            out_command[i][eos_idx_cmd - 1] = 5
