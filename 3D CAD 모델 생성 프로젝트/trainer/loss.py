...
class CADLoss(nn.Module):
    ...
    def forward(self, output, epoch):
        ...
        if self.use_VAE:
            beta = self.beta_end + (self.beta_start - self.beta_end)*(self.beta_rate)**epoch
            loss_kld = output['KLD_loss'] * beta

            res = {"loss_cmd": loss_cmd, "loss_args": loss_args, 'loss_kld': loss_kld}
