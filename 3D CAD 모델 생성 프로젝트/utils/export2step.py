...
for path in out_paths:
    ...
    try:
        if args.form == "h5":
            with h5py.File(path, 'r') as fp:
                ...
                gt_out_shape = vec2CADsolid(gt_vec)
        else:
    ...
    if args.filter:
        ...
        gt_analyzer = BRepCheck_Analyzer(gt_out_shape)
        if not analyzer.IsValid():
            print("pred detect invalid.")
            continue
        if not gt_analyzer.IsValid():
            print("gt detect invalid.")
            continue
    
    name = path.split("/")[-1].split(".")[0]
    save_path = os.path.join(save_dir, name + "_pred.step")
    write_step_file(out_shape, save_path)
    if not_format_h5 == 1:
        save_path = os.path.join(save_dir, name + "_real.step")
        write_step_file(gt_out_shape, save_path)
        not_format_h5 = 0
