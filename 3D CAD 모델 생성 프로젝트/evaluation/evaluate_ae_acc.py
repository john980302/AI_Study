...
for name in tqdm(filenames):
    ...
    # 틀린 것만 출력
    if cmd_acc != 1 or param_acc != 1:
        out_cmd_np.append(out_cmd)
        gt_cmd_np.append(gt_cmd)
        out_param_np.append(out_param)
        gt_param_np.append(gt_param)
        
    
    # 시퀀스 길이에 따른 acc 계산
    if len(gt_cmd) > cur_max_seqlen:
        param_acc_seq += [0 for i in range(len(gt_cmd) + 1 - cur_max_seqlen)]
        
        seqlen_cnt += [1e-6 for i in range(len(gt_cmd) + 1 - cur_max_seqlen)]
        
        cur_max_seqlen = len(gt_cmd)
    
    param_acc_seq[len(gt_cmd)] += param_acc
    seqlen_cnt[len(gt_cmd)] += 1

# 시퀀스 길이에 따른 acc 계산   
param_acc_seq = np.array(param_acc_seq)
seqlen_cnt = np.array(seqlen_cnt)
    
param_acc_seq /= seqlen_cnt       

# numpy 파일 저장
out_cmd_np = np.array(out_cmd_np)
gt_cmd_np = np.array(gt_cmd_np)
out_param_np = np.array(out_param_np)
gt_param_np = np.array(gt_param_np)
np.savez('f_acc', out_cmd=out_cmd_np, gt_cmd=gt_cmd_np, out_param=out_param_np, gt_param=gt_param_np)

# 파일 저장
save_path = "./acc_stat_ours.txt"
fp = open(save_path, "w")
...
for i in range(each_param_acc.shape[0]):
    ...
    k = k + np.array(each_param_acc[i])
    l = l + np.array(args_mask[i])
print("param acc mean: ", k / l, file=fp)

# 시퀀스 길이에 따른 acc 계산
print('Param ACC(Sequence Length): ', param_acc_seq, file=fp)
